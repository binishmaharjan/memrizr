...
func generateIDToken(u *model.User, key *rsa.PrivateKey, exp int64) (string, error) {
	...
	tokenExp := unixTime + exp // 15 minutes for current time
...
func validateIDToken(tokenString string, key *rsa.PublicKey) (*idTokenCustomClaims, error) {
	// claims *IDTokenCustomClaims = &IDTokenCustomClaims{} //
	// claims is assigned a value that is stored at address of empty IDTokenCustomClaims struct
	claims := &idTokenCustomClaims{}
	...
	if !ok {
		return nil, fmt.Errorf("ID token valild but couldn't parse claims")
	}
...
func validateRefreshToken(tokenString string, key string) (*refreshTokenCustomClaims, error) {
	...
	if !ok {
		return nil, fmt.Errorf("refresh token valid but could not parse claims")
	}
